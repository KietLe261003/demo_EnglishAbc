...
interface CalenderProps{
    dataCalender: Array<number>,
    setDataCalender: React.Dispatch<React.SetStateAction<number[]>>
}
const CalendarComponent: React.FC<CalenderProps> = ({dataCalender,setDataCalender}) => {
    const [currentYear, setCurrentYear] = useState<number>(new Date().getFullYear());
    const [currentMonth, setCurrentMonth] = useState<number>(new Date().getMonth());
    const [calendarDays, setCalendarDays] = useState<JSX.Element[]>([]);
    const [modalVisible, setModalVisible] = useState<boolean>(false);
    const [selectedDate, setSelectedDate] = useState<string>('');
    const [currentDayChosse, setCurrentDaySchosse] = useState<number>(new Date().getDate());
        ...
        const days = Array.from({ length: daysInMonth }, (_, index) => {
            ...
            return (
              ...
              >
                ...
                ) : dataCalender.find((item) => item === day) !== undefined ? (
    ...
    const chosseDay = ()=>{     
        setDataCalender([...dataCalender,currentDayChosse]);
        handleCloseModal();
    }
    useEffect(() => {
        generateCalendar(currentYear, currentMonth);
    }, [currentYear, currentMonth,dataCalender]);
    return (
      <>
        <div className='bg-slate-50 shadow-lg rounded-3xl overflow-hidden w-full'>
          <div className='flex items-center justify-between px-6 py-3 '>
            <button
              ...
              className='text-black bg-gray-300 p-3 rounded-full'
            >
              <IconArrowLeft color='black' />
            </button>
            <h2 className='text-black text-[24px] font-bold'>{`${new Intl.DateTimeFormat(
              'en-US',
              { month: 'long', year: 'numeric' },
            ).format(new Date(currentYear, currentMonth))}`}</h2>
            <button
          ...
          <div className='grid grid-cols-7 gap-2 p-4'>{calendarDays}</div>
        </div>
        {modalVisible && (
              ...
              <div className='modal-content py-4 text-left px-6'>
                ...
                <div className='border-t px-4 py-2 flex justify-end gap-3'>
                    <Button variant='secondary' onClick={handleCloseModal}>Hủy</Button>
                     <Button onClick={chosseDay}>Chọn</Button>
                </div>
              </div>
            </div>
          </div>
